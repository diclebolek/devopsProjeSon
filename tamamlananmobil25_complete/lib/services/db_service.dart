...
class DbService {
  ...
  // İŞLETME OKUMA
  // Veritabanındaki eski "Sirius" markası yerine güncel "Orion" adı gösterilir.
  static const String brandName = 'Orion';

  static Map<String, dynamic> _applyBrandName(Map<dynamic, dynamic> row) {
    final map = Map<String, dynamic>.from(row);
    final name = map['isim'];
    if (name is String && name.contains('Sirius')) {
      map['isim'] = name.replaceAll('Sirius', brandName);
    }
    return map;
  }
  ...
  static Future<List<Map<String, dynamic>>> getIsletmeler({String? tip}) async {
    try {
      ...
      return (data as List)
          .map((row) => _applyBrandName(row as Map))
          .toList();
    } catch (e) {
